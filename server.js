...
const readline = require('readline');const express = require('express');
const http = require('http');
const { Server } = require('socket.io');
const { WebcastPushConnection } = require('tiktok-live-connector');
const readline = require('readline');
const fs = require('fs'); // Dosya okuma kütüphanesi eklendi

const app = express();
const server = http.createServer(app);
const io = new Server(server);

// --- SORULARI DOSYADAN ÇEK ---
let questions = [];
try {
    const data = fs.readFileSync('./questions.json', 'utf8');
    questions = JSON.parse(data);
    console.log(`${questions.length} adet soru başarıyla yüklendi.`);
} catch (err) {
    console.error("Sorular yüklenirken hata oluştu! questions.json dosyasını kontrol et.");
}
